...
dataset_sel = 0; % 0 = unsw-nb15, 1 = nslkdd
partition_sel = 1; % 0 = new, random partition, 1 = original partition
LT    = 64; % layer size
SEL_T = 1; % sel = 1 Ripple Arch; sel = 0 Straight-Forward MPBNN code generation

if dataset_sel == 0
    INS = 593; % inputs 593 or 291 for UNSW and KDD, respectively.
else
    INS = 291; % inputs 593 or 291 for UNSW and KDD, respectively.    
end
...
for la = 0:1
    ...
    if partition_sel == 0
        if dataset_sel == 0
            pathw = strcat('Net_Params\dumpUNSW_RAND_w_',num2str(la),'.txt');
            patht = strcat('Net_Params\dumpUNSW_RAND_t_',num2str(la),'.txt');
        else  
            pathw = strcat('Net_Params\dumpKDD_RAND_w_',num2str(la),'.txt');
            patht = strcat('Net_Params\dumpKDD_RAND_t_',num2str(la),'.txt');
        end
    else
        if dataset_sel == 0
            pathw = strcat('Net_Params\dumpUNSW_FULL_w_',num2str(la),'.txt');
            patht = strcat('Net_Params\dumpUNSW_FULL_t_',num2str(la),'.txt');
        else  
            pathw = strcat('Net_Params\dumpKDD_FULL_w_',num2str(la),'.txt');
            patht = strcat('Net_Params\dumpKDD_FULL_t_',num2str(la),'.txt');
        end        
    end
    ...
    if sel == 1
        % Optimize Order
        ordered = hamming_genetic(w,2500,1000);
        word = w(ordered,:);
    end
% return 
    if sel == 1
        ...
        cat_path = strcat('arch_',num2str(SEL_T),'_data_',num2str(dataset_sel),'_partition_',num2str(partition_sel),'_layer_',num2str(la),'.txt'); 
